...
public class PacmanNN implements GameController,Comparable<PacmanNN>{
	...
	public PacmanNN(int inputDim, int hiddenDim, int outputDim) {
	...
	public PacmanNN(int inputDim, int hiddenDim, int outputDim, double[] values) {
		this.inputDim = inputDim;
		this.hiddenDim = hiddenDim;
		this.outputDim = outputDim;
		hiddenWeights = new double[inputDim][hiddenDim];
	...
	private void initializeParameters() {
		...
		for (int i = 0; i < hiddenDim; i++) {
			for (int j = 0; j < outputDim; j++) {
				outputWeights[i][j] = (Math.random() * 5) -1;
			}
		}
		for (int i = 0; i < hiddenDim; i++) {
			hiddenBiases[i] = (Math.random() * 5) -1;
		}
		for (int i = 0; i < outputDim; i++) {
			outputBiases[i] = (Math.random() * 5) -1;
		}
	}
	...
	public int nextMove(int[] currentState) {
		...
		double max=getMax(output);
		if(max==output[1]) return (int)1;
		if(max==output[2]) return (int)2;
		if(max==output[3]) return (int)3;
		if(max==output[4]) return (int)4;
		return 0;
	}
	
	private double getMax(double[] currState) {
		double max=0;
		for(int i=0;i<currState.length;i++){
			if(currState[i]>max) {
                max=currState[i];
	...
	@Override
	public int compareTo(PacmanNN o) {
		return Double.compare(o.fitness, fitness);
	}
	
	public void setFitness(int seed) {
		PacmanBoard b = new PacmanBoard(this, false, seed);
		b.runSimulation();
